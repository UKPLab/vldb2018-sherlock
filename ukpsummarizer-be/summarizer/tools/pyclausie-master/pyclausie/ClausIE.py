...
class ClausIE:
    ...
    def __init__(self, jar_filename=None, download_if_missing=False,
        ...
        if self.jar_filename is None:
            ...
            parse_models = self.setup_and_get_default_path('clausie/clausie_lib/stanford-parser-3.6.0-models.jar')
            parser_file = self.setup_and_get_default_path("clausie/clausie_lib/stanford-parser.jar")

            self.class_path = "%s:%s:%s" % (self.setup_and_get_default_path("clausie/"), parser_file, parse_models)
