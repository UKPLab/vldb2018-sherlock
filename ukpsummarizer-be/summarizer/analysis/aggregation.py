...
class Aggregator(object):
    ...
    def get_summaries(self, cluster_id, user_id):
        ...
        index = np.argmax(np.array(self.R2_scores[cluster_id][user_id][1:]))
        print self.R2_scores[cluster_id][user_id][0], self.R2_scores[cluster_id][user_id][1], self.R2_scores[cluster_id][user_id][index+1]  
        print 'UB summary:\n', ub_summary
        print 'SOA summary:\n', soa_summary
    ...
    def print_results(self):
        ...
        for cluster in range(num_clusters):
            ...
            for user in range(no_users):
                ...
                accepts.append(sum(self.no_accepts[cluster][user][:index]))
                rejects.append(sum(self.no_rejects[cluster][user][:index]))
        ...
        for iteration in range(0,len(self.rows)):
            ...
            for user in range(self.users):
                ...
                if val != "":
                    ...
                    summaries[user].append(row[6+index])
