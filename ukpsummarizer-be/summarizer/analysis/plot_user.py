...
class Plot(object):
    ...
    def plot_scores(self, labels, scores, filename):
        self.users = 2
        f, axis = plt.subplots(2, sharex=True, sharey=True, figsize=(4, 6))
        colors = ['g','b','r', 'y']
        linestyles = ['->', '-o', '-x', '-']
        iterations= 8
        ...
        plt.ylabel('ROUGE 2', weight='bold', fontsize=13, horizontalalignment='center')
        plt.yscale("linear")
...
if __name__ == '__main__':
    ...
    if size!= None:
        data_path = '../data/scores/%s/%s_%s_%s' % (data_set, methods[0], data_set, size)
    else:
        data_path = '../data/scores/%s/%s_%s' % (data_set, methods[0], data_set)
