...
class CorpusCleaner(object):
    ...
    def parse_xml_DIP(self, data_file, language='english'):
        e = ET.parse(data_file)
        root = e.getroot()
        data = []
        for document in root.find('documents').findall('document'):
            doc_id = document.attrib.get('clueWebID')
            doc_sents = []
            print('Document_ID', doc_id)
            for sentence in document.find('sentences').findall('s'):
                relevant = sentence.attrib.get('relevant')
                if relevant == 'true':
                    print(relevant)
                    doc_sents.append(sentence.find('content').text)
            data.append([doc_id, doc_sents])
        return data
    ...
    def load_processDIP(self, ctopic):
        cluster_path = '%s/%s' % (self.corpus_path, ctopic)
        print(cluster_path)
        docs_data = self.parse_xml_DIP(cluster_path, language='english')
        summaries_data = [['ID', ['DUMMY TEXT']]]
        return docs_data, summaries_data
    ...
    def cleanWiki_data(self, parse_type):
        for ctopic in os.listdir(self.corpus_path):
    ...
    def cleanDIP_data(self, parse_type):
        for ctopic in os.listdir(self.corpus_path):
            docs, summaries = self.load_processDIP(ctopic)
            ctopic = ctopic[:ctopic.rfind('.')]
    ...
    def parse_xml(self, file_data, filename="unknown filename"):
        ...
        if self.corpus_name == "DUC2002" or self.corpus_name == "DUC2001" or self.corpus_name == "DUC2006" or self.corpus_name.startswith("DUC2007"):
            text = ""
            if self.corpus_name == 'DUC2006' or self.corpus_name.startswith('DUC2007'):
    ...
    def add_dataset_index(self, lang):
        filename = path.normpath(path.join(self.cleaned_path, self.corpus_name, "index.json"))

        v = {}
        v["language"] = lang
        v["corpus"] = self.corpus_name
        v["dataset"] = self.corpus_name
        text =json.dumps(v)
        create_dir(path.normpath(path.join(self.cleaned_path, self.corpus_name)))
