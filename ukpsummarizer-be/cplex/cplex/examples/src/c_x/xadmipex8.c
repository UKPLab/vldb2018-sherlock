...
/* admipex5.c -- Solve a facility location problem with cutting plane using the
 ...
 */
static int
disaggregate(CPXCALLBACKCONTEXTptr context, const char *const*cname)
{
   int status;
   CPXDIM j, c;
   double x[NVARS];
...
callback (CPXCALLBACKCONTEXTptr context, CPXLONG where, void* userdata)
{
   ...
   if ( where == CPX_CALLBACKCONTEXT_CANDIDATE ) {
...
main(int argc, char **argv)
{
   ...
   if ( usecallback ) {
      /* Set up the user cuts. We change the where parameter with a bitwise or, we populate
   ...
   if ( lazy ) {
      /* Setup for the lazy constraint. We just change the where parameter with a bitwise or.
       */
